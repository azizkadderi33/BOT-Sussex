module.exports.config = {
  ...
  module.exports.run = async function ({ event, api , args, Users}){
    ...
    var msg = `[⚜️] صور عشوائية  [⚜️]\n\n`;
    var x = 1;
    for (var i = limit * (page - 1); i < limit * (page - 1) + limit; i++) {
        if (i >= b) break;
        msg += `[⚜️] ${i+1} [⚜️] → ${name[i]}\n\n`;
    }
    msg += `[⚜️]→ الصفحة (${page}/${numPage})\n[⚜️]→ استخدم ${global.config.PREFIX}${this.config.name} <رقم الصفحة>\n\n[⚜️]→ من فضلك ${name1} قم بالرد على الرسالة بالرقم اللذي اخترته!!!`;
  ...
  module.exports.handleReply = async function ({ event, api , args, handleReply, Users}){
    ...
    return api.sendMessage({body: `[⚜️] تم الامر بنجاح [⚜️]\n[❤️] هذه هي صورتك ${name} !!!`, attachment : download}, event.threadID)
}
    }
}