...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  const msg = ({
    body: "رد على هذه الرسالة برقم الستايل",
    attachment: o
  })
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  if (handleReply.step == 1) {
    if(isNaN(event.body)) return api.sendMessage('هذا ليس رقما :>', event.threadID)
    api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`رقم ستايلك هو ${event.body} رد على هذه الرسالة برقم اخر`, threadID, function(err, info) {
      return global.client.handleReply.push({
        step: 2,
        name: "بنر3",
        author: senderID,
    ...
    api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`رقم الشخصية اللتي اخترتها هو ${event.body} رد على هذه الرسالة بادخالك لاسمك الاساسي`, threadID, function(err, info) {
      return global.client.handleReply.push({
        step: 3,
        name: "بنر3",
        author: senderID,
    ...
    api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`اسمك هو ${event.body} رد على هذه الرسالة بادخال لقبك`, threadID, function(err, info) {
      return global.client.handleReply.push({
        step: 4,
        name: "بنر3",
        author: senderID,
    ...
    api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`لقبك هو ${event.body}, رد على هذه الرسالة باللون اللذي تريده واكتبه بالانجليزية (اذا اردت استخدم اللون الافتراضي قم بكتابة "no")`, threadID, function(err, info) {
      return global.client.handleReply.push({
        step: 5,
        name: "بنر3",
        author: senderID,
