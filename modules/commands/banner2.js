...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  if(args[0] == "find" || args[0] == "tìm"){
   ...
   return api.sendMessage("قم باختيار رقم", event.threadID, (err, info) => {
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  console.log(lengthchar.length)
    ...
    if(handleReply.step == 1){
      ...
      return api.sendMessage(`لقد اخترت شخصية رقمها هو ${event.body}, من فضلك قم بادخال اسمك`, event.threadID, (err, info) => {
    return global.client.handleReply.push({
      step: 2,
      name: this.config.name,
      chartid: event.body,
      ...
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`لقد اخترت اسم  ${event.body} , قم بالرد على هذه الرسالة باللون اللذي ترغبه واكتبه بالانجليزية`, event.threadID, (err, info) => {
    return global.client.handleReply.push({
      step: 3,
      name: this.config.name,
