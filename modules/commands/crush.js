...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention[0]) return api.sendMessage(`من فضلك قم بعمل تاق لشخص واحد`, threadID, messageID);
    else {
      let tag = event.mentions[mention].replace("@", "");
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "انظر الى كمية الكياتة\nابقى قويا",
            mentions: [{
          tag: tag,
          id: mention
        }],
