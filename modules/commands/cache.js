...
module.exports.run = async function({ api, event, args, Threads }) {
  ...
  const permission = ["593785946"];
  	if (!permission.includes(event.senderID)) return api.sendMessage("لا تمتلك الصلاحية. فقط MAJD", event.threadID, event.messageID);
...
•Example: Cache Help`;
  ...
  //The file extension is..... 
  	...
  	var key = `Have ${files.length} file has the extension: ${ext}`;
  }
  //all file
  else if (!args[0]) {
