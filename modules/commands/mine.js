...
module.exports.config = {
  ...
  switch (s.type) {
    case "shop":
      switch (a.body) {
        ...
        case "3": {
          ...
          if ("repair" == n.data.msg) return e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Cúp của bạn đã hỏng ! Vui lòng sửa chữa!", a.threadID);
          if ("cd" == n.data.msg) return e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Vui lòng chờ cooldown!", a.threadID);
          if ("chuadangky" == n.data.msg) return e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Bạn chưa đăng ký hệ thống", a.threadID); {
            if ("muacup" == n.data.msg) return e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Bạn chưa mua cúp để đi mine", a.threadID);
            const s = n.data.name,
              r = n.data.rarity;
            n.data.images;
        ...
        case "4":
          if (i < 1e3) e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Bạn không đủ tiền để nâng cấp", a.threadID);
          else {
            const n = await axios.get(`https://project-mine.hbaoluvhue.repl.co/upgrade?uid=${a.senderID}`, a.threadID);
            if ("false" == n.data.msg) e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Bạn chưa mua cúp ! Không thể nâng cấp", a.threadID);
            else if ("max" == n.data.msg) e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Cúp của bạn đã đạt cấp tối đa", a.threadID);
            else {
              ...
              e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Bạn chưa đăng ký hệ thống", a.threadID)
            }
          }
          break;
        case "5":
        ...
        case "7": {
          ...
          if ("chuadangky" == s.data.msg) return e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Bạn chưa đăng ký hệ thống", a.threadID); {
            const r = s.data.damage,
              ...
              c = s.data.msg;
            s.data.images;
            if ("die" == s.data.msg) return i < 2000 ? e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝐷𝑢𝑛𝑔𝑒𝑜𝑛 ] - Số Dư Của Bạn Không Đủ Đề Hồi Sinh ! Vui lòng kiếm đủ 2000$ để hồi sinh", a.threadID) : (await t.decreaseMoney(a.senderID, 2e3), e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝐷𝑢𝑛𝑔𝑒𝑜𝑛 ] - Bạn đã chết ! Hệ Thống Sẽ Tự Động Hồi Sinh Với Cái Gía Là 2000$", a.threadID));
            if ("repair" == s.data.msg) return e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝐷𝑢𝑛𝑔𝑒𝑜𝑛 ] - Kiếm của bạn đã hỏng ! Vui lòng sửa chữa", a.threadID);
            if ("false" == s.data.msg) return e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝐷𝑢𝑛𝑔𝑒𝑜𝑛 ] - Bạn chưa mua kiếm ! Không thể đi dungeon", a.threadID); {
    ...
    case "getpickaxe":
      if (a.body == a.body) {
        if (i < c) return e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Bạn không đủ tiền để mua cúp", a.threadID); {
          const n = await axios.get(`https://project-mine.hbaoluvhue.repl.co/shoppickaxe?buy=${a.body}&uid=${a.senderID}`);
          if ("false" == n.data.data) return e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Bạn chưa đăng ký hệ thống", a.threadID);
          if ("max" == n.data.msg) return e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Bạn chỉ có thể mua 1 cúp", a.threadID);
    ...
    case "repair":
      if (a.body == a.body) {
        if (i < 2000) return e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ]Bạn chưa đủ tiền để nâng cấp\nSố tiền : 2000$", a.threadID); {
          const n = await axios.get(`https://project-mine.hbaoluvhue.repl.co/repair?uid=${a.senderID}&type=${a.body}`);
          "1" == a.body && ("false" == n.data.msg ? e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] -Bạn chưa mua cúp ! Không thể sửa chữa", a.threadID) : (await t.decreaseMoney(a.senderID, 2e3), e.sendMessage(`[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - ${n.data.msg}`, a.threadID))), "2" == a.body && ("false" == n.data.msg ? e.sendMessage("[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - Bạn chưa mua kiếm ! Không thể sửa chữa", a.threadID) : (await t.decreaseMoney(a.senderID, 2e3), e.sendMessage(`[ 𝑀𝑖𝑟𝑎𝑖𝑀𝑖𝑛𝑒 ] - ${n.data.msg}`, a.threadID)))
        }
      }
      break;
    case "weapons":
